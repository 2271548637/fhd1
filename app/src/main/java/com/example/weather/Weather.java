...
public class Weather extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(databaseid ==  tranformat ){
            sign = 1;//标识符置1：直接解析从数据库读写的数据
            showResponse(databasedata);//解析并展示数据
        }else {
            sign = 0;//标识符置0：需要访问api,重新写入数据库
            ...
            public void run() {
                try{
                    ...
                    Request request = new Request.Builder()
                            .url("http://t.weather.itboy.net/api/weather/city/"+researchcitycode)//访问API
                            .build();
                    Response response = client.newCall(request).execute();
                    String responseData = response.body().string();
                    Log.d("data is", responseData);
                    showResponse(responseData);//将数据展现出来
                }catch (Exception e){
                    e.printStackTrace();
                }
            }
